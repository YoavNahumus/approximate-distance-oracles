 ...
 * @param doEdgeSet A boolean value indicating whether the algorithm should add the edge set to A1 or not.
 * @param connectMiddleLevel A boolean value indicating whether the algorithm should connect A_(k/2).
 */
ADO::ADO(Graph* graph, int k, bool doEdgeSet, bool connectMiddleLevel) : graph(graph), k(k), doEdgeSet(doEdgeSet), connectMiddleLevel(connectMiddleLevel) {
...
/**
 * Builds the cluster.
 * @param i The level of the hierarchy of the cluster.
...
void ADO::buildCluster(int i, pair<const vertex, unordered_map<vertex, distance>*>* q) {
    ...
    if ((i == k - 1 || hierarchy[i + 1]->count(q->first) == 0) && (i != (k - 1) / 2 || !connectMiddleLevel)) {
    ...
    } else if (i == (k - 1) / 2 && connectMiddleLevel) {
